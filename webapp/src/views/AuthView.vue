		...
		<div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
			<div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
				<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
					<h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white text-center">Авторизация</h1>
					<div class="space-y-4 md:space-y-6" action="#">
						<UILabeledInput v-model="login" type="text" property="login" placeholder=""> Почта </UILabeledInput>
						<UILabeledInput v-model="password" type="password" property="password" placeholder=""> Пароль </UILabeledInput>
						<UIButton @click="auth" :disabled="loading || !isInputsSet" classExtension="w-full px-5 py-2.5"> Войти </UIButton>
					</div>
				</div>
			</div>
		</div>
