			...
			<div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
				<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
					...
					<div v-if="stage == STAGE.SETUP_STARTER" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						...
						<UILabeledInput type="text" v-model="author"> Как вас зовут? (автор эпитафии, ФИО) </UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="changeStage(STAGE.SETUP_PERSON)" :disabled="author.length < 1 || !wrappedPages.hasOwnProperty(selectedPage)"> Продолжить </UIButton>
					</div>
					<div v-if="stage == STAGE.SETUP_PERSON" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						<div>
							<UILabel> Укажите пол </UILabel>
							<UIDropdown v-model="sex" :options="{ мужского: 'Мужской', женского: 'Женский' }"> Пол не выбран </UIDropdown>
						</div>
						<UILabeledInput type="text" v-model="name"> Как {{ sex == 'мужского' ? 'его' : 'ее' }} звали? </UILabeledInput>
						<div>
							<UILabel> Когда {{ sex == 'мужского' ? 'он родился' : 'она родилась' }}? </UILabel>
							<VueDatePicker
								v-model="dates[0]"
						...
						<div>
							<UILabel> Когда {{ sex == 'мужского' ? 'он умер' : 'она умерла' }}? </UILabel>
							<VueDatePicker
								v-model="dates[1]"
								:enable-time-picker="false"
								:min-date="dates[0]"
						...
						<UIButton classExtension="w-full py-2.5" @click="changeStage(STAGE.SETUP_PLACES)" :disabled="name.length < 1"> Продолжить </UIButton>
					</div>
					<div v-if="stage == STAGE.SETUP_PLACES" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						<UILabeledInput type="text" v-model="places[0]"> Где {{ sex == 'мужского' ? 'он родился' : 'она родилась' }}? </UILabeledInput>
						<UILabeledInput type="text" v-model="places[1]"> Где {{ sex == 'мужского' ? 'он умер' : 'она умерла' }}? </UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="changeStage(STAGE.SETUP_RELATIVES)" :disabled="places[0].length < 1 || places[1].length < 1"> Продолжить </UIButton>
					</div>
					<div v-if="stage == STAGE.SETUP_RELATIVES" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						<UILabeledInput type="text" v-model="partner"> Укажите {{ sex == 'мужского' ? 'супругу' : 'супруга' }} (при наличии) </UILabeledInput>
						<UILabeledInput type="text" :textarea="true" v-model="children"> Укажите детей (при наличии) </UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="changeStage(STAGE.SETUP_EDUCATION)"> Продолжить </UIButton>
					</div>
					<div v-if="stage == STAGE.SETUP_EDUCATION" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						<UILabeledInput type="text" v-model="citizenship"> В какой стране {{ sex == 'мужского' ? 'он жил' : 'она жила' }}? </UILabeledInput>
						<UILabeledInput type="text" v-model="education"> Какое у {{ sex == 'мужского' ? 'него' : 'нее' }} было образование? </UILabeledInput>
						<UILabeledInput type="text" v-model="career"> Какой у {{ sex == 'мужского' ? 'него' : 'нее' }} был род деятельности? </UILabeledInput>
						<UILabeledInput type="text" :textarea="true" v-model="achievments"> Какие были награды, премии, достижения? </UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="next" :disabled="citizenship.length < 1"> Продолжить </UIButton>
					</div>
					...
					<div v-if="stage == STAGE.QUESTION" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						<UILabeledInput type="text" class="animate animate-fade animate-ease-in-out animate-duration-350 animate-once" v-model="answers[index]">
							{{ questions[index] }}
						</UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="next" :disabled="answers[index].length < 1"> Продолжить </UIButton>
						<p class="text-sm font-light text-gray-500 dark:text-gray-400 text-center !mt-4">
							<a href="#" class="font-medium text-primary-600 hover:underline dark:text-primary-500" @click="skip"> Пропустить вопрос<br /><br /> </a>
							<a href="#" class="font-medium text-primary-600 hover:underline dark:text-primary-500" @click="back" v-if="index != 0"> Вернуться на предыдущий </a>
						</p>
					</div>
					...
					<div v-if="stage == STAGE.VIEW" class="space-y-2 md:space-y-4 animate animate-fade animate-ease-in-out animate-duration-250 animate-once text-white text-center">
						<div class="grid gap-2 mb-2 grid-cols-2">
							<UIButton color="warning" classExtension="py-2.5" @click="regenerate"> Пересоздать </UIButton>
							<UIButton color="danger" classExtension="py-2.5" @click="reset"> Сбросить </UIButton>
						</div>
						<UILabeledInput :textarea="true" type="text" class="animate animate-fade animate-ease-in-out animate-duration-350 animate-once" v-model="results[0]">
							Предложенная эпитафия №1:
						</UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="acceptEpitaphy(0)"> Принять первую эпитафию </UIButton>
						<UILabeledInput :textarea="true" type="text" class="animate animate-fade animate-ease-in-out animate-duration-350 animate-once" v-model="results[1]">
							Предложенная эпитафия №2:
						</UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="acceptEpitaphy(1)"> Принять вторую эпитафию </UIButton>
					</div>
					<div v-if="stage == STAGE.PROMPT_BIOGRAPHY" class="space-y-2 md:space-y-4 animate animate-fade animate-ease-in-out animate-duration-250 animate-once text-white text-center">
						<UILabel> Хотите ли вы создать биографию для {{ name }}? </UILabel>
						<div class="grid gap-2 mb-2 grid-cols-2">
							<UIButton color="danger" classExtension="py-2.5" @click="finish"> Нет </UIButton>
							<UIButton classExtension="py-2.5" @click="changeStage(STAGE.SELECT_BIOGRAPHY)"> Да </UIButton>
						</div>
					</div>
					<div v-if="stage == STAGE.SELECT_BIOGRAPHY" class="space-y-2 md:space-y-4 animate animate-fade animate-ease-in-out animate-duration-250 animate-once text-white text-center">
						<UILabel> До какого периода вы хотите создать биографию? </UILabel>
						<UIButton classExtension="py-2.5" @click="selectBiography(1)"> До детства/юношества (Одна часть) </UIButton>
						<UIButton classExtension="py-2.5" @click="selectBiography(2)"> До средних лет (Две части) </UIButton>
						<UIButton classExtension="py-2.5" @click="selectBiography(3)"> До старости (Три части) </UIButton>
					</div>
					<div v-if="stage == STAGE.BIOGRAPHY_QUESTION" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						...
						<UILabeledInput type="text" class="animate animate-fade animate-ease-in-out animate-duration-350 animate-once" v-model="biography[biographyIndex].answers[index]">
							{{ biography[biographyIndex].questions[index] }}
						</UILabeledInput>
						<UIButton classExtension="w-full py-2.5" @click="next" :disabled="biography[biographyIndex].answers[index].length < 1"> Продолжить </UIButton>
						<p class="text-sm font-light text-gray-500 dark:text-gray-400 text-center !mt-4">
							<a href="#" class="font-medium text-primary-600 hover:underline dark:text-primary-500" @click="skip"> Пропустить вопрос<br /><br /> </a>
							<a href="#" class="font-medium text-primary-600 hover:underline dark:text-primary-500" @click="back" v-if="index != 0"> Вернуться на предыдущий </a>
						</p>
					...
					<div v-if="stage == STAGE.VIEW_BIOGRAPHY" class="space-y-4 md:space-y-6 animate animate-fade animate-ease-in-out animate-duration-250 animate-once">
						<div class="grid gap-2 mb-2 grid-cols-2">
							<UIButton color="warning" classExtension="py-2.5" @click="generateBiography"> Пересоздать </UIButton>
							<UIButton color="danger" classExtension="py-2.5" @click="changeStage(STAGE.PROMPT_BIOGRAPHY)"> Сбросить </UIButton>
						</div>
						<UILabel> Предложенная биография </UILabel>
						<div style="max-height: 50vh; overflow: auto">
							<div v-for="(item, i) in biography" :key="item.stage" :class="[item.header.length < 1 ? 'hidden' : '']">
								<UILabeledInput type="text" v-model="item.header"> Заголовок: </UILabeledInput>
								<UILabeledInput :textarea="true" type="text" v-model="item.text"> Текст: </UILabeledInput>
							</div>
							<UILabeledInput :textarea="true" type="text" v-model="biographyEnding"> Заключение: </UILabeledInput>
						</div>
						<UIButton classExtension="w-full py-2.5" @click="acceptBiography"> Принять биографию </UIButton>
					</div>
...
export default {
	...
	methods: {
		/**
		 * A function to get questions from MemoryService.
		 */
		getQuestions() {
			MemoryService.getQuestions(
				MODEL,
				this.name,
				this.sex,
				this.dates[0],
		...
		/**
		 * A function to handle the next stage of the process.
		 */
		next() {
		...
		/**
		 * A function to check the end condition for biography generation and proceed accordingly.
		 */
		checkEnd() {
		...
		/**
		 * A function to generate epitaphs and set the stage of the process.
		 */
		generate() {
		...
		/**
		 * Generates biography questions based on the biography index.
		 */
		generateBiographyQuestions() {
			this.stage = STAGE.AWAIT_QUESTION;
			let type = this.biographyIndex == 0 ? 'youth' : this.biographyIndex == 1 ? 'middle_age' : 'old_age';
			MemoryService.getBiographyQuestions(
				MODEL,
				type,
				this.name,
				this.sex,
		...
		/**
		 * Generates the next biography based on the provided index.
		 *
		 * @param {number} i - The index for generating the next biography
		 */
		generateNextBiography(i) {
			if (i == 3) {
				MemoryService.getBiographyShort(
					MODEL,
					this.biography,
					this.name,
					this.sex,
			...
			if (amount > 0) {
				let type = i == 0 ? 'youth' : i == 1 ? 'middle_age' : 'old_age';
				MemoryService.getBiography(
					MODEL,
					type,
					this.name,
					this.sex,
					...
					this.dates[1],
					questions,
					i == 0 ? 'Пустая биография' : this.biography[i - 1].text,
					data => {
		...
		/**
		 * Sets the stage to APPLYING, creates options for a request, and sends the page to ExternalService.
		 */
		finish() {
		...
		/**
		 * A function that splits a full name into its parts.
		 *
		 * @param {string} name - The full name to split.
		 * @return {Object} An object containing the firstName, patronymic, and lastName parts of the full name.
		 */
		splitName(name) {
	 ...
	 */
	mounted() {
		if (this.$route.query.path) this.$router.push({ path: this.$route.query.path });
